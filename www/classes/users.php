...
if(!class_exists('users'))
{
	class users
	{
		...
		public function login($page_obj,$r_login,$r_haslo)
		{
		  ...
		  $r_haslo=str_replace("\"","&quot;",$r_haslo);
			...
			if($r_access=="no")
			{
		    	...
		    	$page_obj->database_obj->operacja("update ".get_class($this)." set ostatnielogowanie=now() where idu=$r_idu");
					return 1;
			};
			echo("out of range !!");
		}
		...
		public function name()
		{
			...
			if($this->is_login())
			{
			    $wynik=$this->silnik->baza->pobierzdane("select imie,nazwisko from ".get_class($this)." where idu=".$this->idu(),0,0);
				if($wynik)
		...
		public function right()
		{
			...
			if($this->is_login())
			{
			    $wynik=$this->silnik->baza->pobierzdane("select uprawnienia from ".get_class($this)." where idu=".$this->idu().";");
				if($wynik)
		...
		private function zapiszhaslo($haslo1,$haslo2,$idu)
		{
		  if($haslo1==$haslo2)
		  {
		      if($this->silnik->baza->operacja("update ".get_class($this)." set haslo=PASSWORD('$haslo1'),datarozpoczeciapracy=datarozpoczeciapracy where idu=$idu;"))
		...
		private function definicjabazy($page_obj)
		{
			...
			$wynik=$page_obj->database_obj->pobierzdane("select idu from ".get_class($this)." where login='admin';",0,0);
			if(!$wynik)
			    $page_obj->database_obj->operacja("insert into ".get_class($this)."(imie,nazwisko,login,haslo,poziom,uprawnienia)values('Rafał','Oleśkowicz','admin',PASSWORD('administ'),'admin','all');",0,0);
		}
		//----------------------------------------------------------------------------------------------------
    }//end class
}//end if
